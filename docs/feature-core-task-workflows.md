...
## Docker Validation Findings and Corrections
...
The next validation successfully applied all PostgreSQL migrations and showed the web container as internally healthy, but the loopback host port was unreachable because the web service was attached only to the internal-only backend network. The Compose design now separates network roles: PostgreSQL remains only on the internal `backend` network, while the web service joins both `backend` and a normal `edge` bridge. This keeps database traffic isolated while allowing the explicitly loopback-bound development web port to function and be tested from the host.
