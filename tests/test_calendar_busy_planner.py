...
def test_derives_only_complement_windows_at_or_above_minimum_duration() -> None:
    start = datetime(2026, 9, 3, 9, 0, tzinfo=timezone.utc)
    busy = context(
        BusyInterval(start + timedelta(hours=1), start + timedelta(hours=2)),
        BusyInterval(start + timedelta(hours=3), start + timedelta(hours=5)),
        BusyInterval(start + timedelta(hours=7, minutes=30), start + timedelta(hours=8)),
    )
    ...
    assert windows == (
        ...
        FreeWindow(start + timedelta(hours=5), start + timedelta(hours=7, minutes=30)),
    )


def test_empty_busy_context_returns_whole_range_as_one_advisory_window() -> None:
    busy = context()

    windows = derive_free_windows(busy, minimum_duration=timedelta(minutes=30))

    assert windows == (FreeWindow(busy.starts_at, busy.ends_at),)


def test_limit_returns_earliest_qualifying_windows_deterministically() -> None:
    start = datetime(2026, 9, 3, 9, 0, tzinfo=timezone.utc)
    busy = context(
        BusyInterval(start + timedelta(hours=1), start + timedelta(hours=2)),
...
def test_rejects_manually_constructed_overlapping_busy_context() -> None:
    start = datetime(2026, 9, 3, 9, 0, tzinfo=timezone.utc)
    busy = context(
    ...
    with pytest.raises(CalendarBusyError, match="strictly ordered and fully merged"):
        derive_free_windows(busy, minimum_duration=timedelta(minutes=30))


def test_rejects_unbounded_or_invalid_planning_parameters() -> None:
    busy = context()

    for duration in (timedelta(minutes=4), timedelta(hours=25)):
        with pytest.raises(CalendarBusyError, match="between 5 minutes and 24 hours"):
            derive_free_windows(busy, minimum_duration=duration)

    for invalid_limit in (0, 33, True):
        with pytest.raises(CalendarBusyError, match="between 1 and 32"):
            derive_free_windows(
                busy,
                minimum_duration=timedelta(minutes=30),
                limit=invalid_limit,
